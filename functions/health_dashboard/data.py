...
Read-only, and a deliberate column allowlist: sensitive fields (medications,
weight, raw payloads) are never selected. (The dashboard is private now, but
the allowlist stays as defense in depth — if the page ever leaks, the queries
still can't return those columns.)
...
WHY user_id IS AN ARGUMENT, NOT A MODULE CONSTANT: the dashboard can switch
between subjects, and @st.cache_data keys its cache on the *arguments* a
function was called with. Passing user_id in means each subject's frames are
cached under their own key, so switching subjects can never serve you another
person's cached data. A module-level constant would be invisible to the cache
...
def _query(sql: str, user_id: str, days: int) -> pd.DataFrame:
    ...
    # Built before the client call, not inline in it: _user_param validates the
    # subject, and Python evaluates `_client()` before a call's arguments — so
    # inlining this would spin up a BigQuery client before a bad id could raise.
    params = [_user_param(user_id),
...
def _user_param(user_id: str):
    """Build the @user_id bind parameter — and the single choke point where
    the subject is validated. Every user-filtered query below gets its user
    parameter from here, so an unknown id raises instead of reaching BigQuery
    (a typo like "vince" fails loudly rather than silently returning zero rows,
    which would otherwise look like "this person has no data")."""
    if user_id not in SUBJECTS:
        raise ValueError(f"unknown subject {user_id!r}; expected one of {SUBJECTS}")
    return bigquery.ScalarQueryParameter("user_id", "STRING", user_id)


def _window_params(user_id: str, start_ts, end_ts):
    """The parameter triple used by every windowed loader below."""
    return [_user_param(user_id),
...
def load_daily(user_id: str, days: int) -> pd.DataFrame:
...
@st.cache_data(ttl=CACHE_TTL_S)
def load_glucose(user_id: str, days: int) -> pd.DataFrame:
...
@st.cache_data(ttl=CACHE_TTL_S)
def load_blood_pressure(user_id: str, days: int) -> pd.DataFrame:
    return _query(f"""
...
@st.cache_data(ttl=CACHE_TTL_S)
def load_meals(user_id: str, limit: int = 200) -> pd.DataFrame:
...
@st.cache_data(ttl=CACHE_TTL_S)
def load_glucose_window(user_id: str, start_ts, end_ts) -> pd.DataFrame:
...
@st.cache_data(ttl=CACHE_TTL_S)
def load_activities_window(user_id: str, start_ts, end_ts) -> pd.DataFrame:
...
@st.cache_data(ttl=CACHE_TTL_S)
def load_bp_window(user_id: str, start_ts, end_ts) -> pd.DataFrame:
    return _query_params(f"""
        SELECT measurement_ts_utc, systolic, diastolic, pulse
        FROM `{DATASET}.omron_bp_daily`
...
def load_hrv_for_sleep_date(user_id: str, sleep_date: str) -> pd.DataFrame:
    ...
    return _query_params(f"""
        ...
        ORDER BY ts
    """, [_user_param(user_id),
          bigquery.ScalarQueryParameter("sleep_date", "DATE", sleep_date)])
